...
class VFHModel:
    ...
    def __init__(self):
        ...
        ## Evaluate argument requirements
        #if np.mod(360.0/alpha) != 0:
        #    raise ValueError("Alpha must define a integer amount of sectors")
        ...
        for i in xrange(WINDOW_SIZE):
            for j in xrange(WINDOW_SIZE):
                if j == WINDOW_CENTER and i == WINDOW_CENTER:
                    continue
                self.active_window[i,j,BETA] = np.degrees(np.arctan2(j-wj0, i-wi0))
                self.active_window[i,j,DIST] = RESOLUTION*np.sqrt(np.square(i-wi0) + np.square(j-wj0))
